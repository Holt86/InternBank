...
public class EsbServiceImplTest {
  ...
  public void testSendRequests() throws IllegalAccessException {
    ...
    // when
    request.setEntityType(RequestType.UPD_ST_CASH_ORDER);
    request.setRequestData(new CashOrder());
    esbService.sendRequest(request);
    requestCount++;
    // then
    ...
    request.setRequestData(new CheckOverLimit());
    esbService.sendRequest(request);
    requestCount++;
    // then
    Assert.assertEquals(putRequests.size(), requestCount);
    ...
    // when
    request.setEntityType(RequestType.GET_WORK_PLACE_INFO);
    request.setRequestData("000");
    esbService.sendRequest(request);
    requestCount++;
    // then
    ...
    String[] requestTypes = new String[] {RequestType.ACCOUNT_20202,
        ...
        RequestType.CREATE_CASH_ORDER, RequestType.UPD_ST_CASH_ORDER,
        RequestType.GET_WORK_PLACE_INFO, RequestType.CHECK_OVER_LIMIT};
